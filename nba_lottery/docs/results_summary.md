...
See `docs/statistical_analysis_plan.md` and `docs/analysis_plan_hashes.txt` for the pre-registered plan.
...
**Caption (confirmatory).** Observed global statistic S = 138.9898 plotted against a Gaussian approximation of the Monte Carlo null distribution over 20 confirmatory-eligible years. p = 0.538281. Pre-trade slot-owner view.

![Figure 2 — per-year NLL contributions](../outputs/figures/fig2_per_year_nll.png)

**Caption (confirmatory, robustness gate input).** Per-year −log p contributions to S. Error bars are Monte Carlo SEs. The robustness gate requires that the top-3 years' share of NLL excess be less than 0.5 (current value: 0.7434).

![Figure 3 — LOYO sensitivity](../outputs/figures/fig3_loyo.png)

**Caption (confirmatory, robustness gate).** p-value of A with each year removed. No single-year removal flips the α=0.05 decision.
...
**Caption (confirmatory).** Observed S vs null mean for each era. No era shows significant deviation from its official process.
...
**Caption (exploratory, effect-size only, pre-trade slot-owner view).** Observed − expected #1-pick wins per team over 20 confirmatory-eligible years. Notable effects by magnitude (all below the frequentist eligibility threshold, so no p-values):
...
**Caption (exploratory).** Observed − expected #1-pick wins by pre-lottery rank.

### Outlier-year detection

![Figure 6 — outlier years](../outputs/figures/fig6_outlier_years.png)

**Caption (exploratory).** Per-year −log p of the observed drawn tuple under each year's null. After BH-FDR correction at q = 0.10, 0 years remain 'significant.' The most improbable uncorrected years are:
...
This does not prove the lottery is fair — it means that if any bias exists, it is below this study's detection threshold. See `docs/limitations.md` for a full accounting of what this study can and cannot establish.
